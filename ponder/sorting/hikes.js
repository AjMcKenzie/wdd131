/* 
onst names = ["James", "bob", "JOSH"]; 
names.sort();
console.log(names);



const ages = [33, 12, 7, 100]; 
ages.sort();
console.log(ages);
...
searchButton.addEventListener("click", e => {
  ...
  // filter results based on what they typed
  const filtteredHikes = hikes.filter(hike => {
    // if(hike.name.toLowerCase().includes(userInput)){
    //   return true;
    // } else {
    //   return false;
    // }
  ...
  // Sort filltered results by difficulty
  filtteredHikes.sort(sortHikesByDifficulty);
});
